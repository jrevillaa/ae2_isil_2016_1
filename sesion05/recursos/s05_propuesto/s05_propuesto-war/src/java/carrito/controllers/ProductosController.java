...
import carrito.fachada.ProductosFacade;
import carrito.fachada.ProductosFacadeLocal;
...
public class ProductosController implements Serializable {
    ...
    @EJB
    private ProductosFacadeLocal ejbFacade;
    ...
    protected void initializeEmbeddableKey() {
        selected.setProductosPK(new carrito.entidades.ProductosPK());
    }

    private ProductosFacadeLocal getFacade() {
        return ejbFacade;
    }
    ...
    public Productos getProductos(carrito.entidades.ProductosPK id) {
    ...
    public static class ProductosControllerConverter implements Converter {
        ...
        carrito.entidades.ProductosPK getKey(String value) {
            carrito.entidades.ProductosPK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new carrito.entidades.ProductosPK();
            key.setCodigo(Integer.parseInt(values[0]));
            key.setTipo(new Character(values[1].charAt(0)));
            return key;
        }

        String getStringKey(carrito.entidades.ProductosPK value) {
